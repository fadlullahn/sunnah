...
import com.example.sunnah.KategoriUbahActivity;
...
import com.example.sunnah.SubDataActivity;
...
public class AdapterDataKategori extends RecyclerView.Adapter<AdapterDataKategori.HolderData> {
    ...
    public AdapterDataKategori(Context ctx, List<DataModelKategori> listData) {
        this.ctx = ctx;
            ...
            ambilData.enqueue(new Callback<ResponseModelKategori>() {
                ...
                public void onResponse(Call<ResponseModelKategori> call, Response<ResponseModelKategori> response) {
                    if (response.isSuccessful() && response.body() != null) {
                        ...
                        editor.putString("xSubKategori", varSubKategori);
                        editor.apply();

                        Intent kirim;
                        if (varSubKategori != null && !varSubKategori.isEmpty() && !varSubKategori.equals("0")) {
                            kirim = new Intent(ctx, SubDataActivity.class);
                        } else {
                            kirim = new Intent(ctx, KategoriUbahActivity.class);
                        }
                        ...
                        Toast.makeText(ctx, "Respon tidak valid dari server.", Toast.LENGTH_SHORT).show();
                    }
                }
